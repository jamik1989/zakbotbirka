...
def _fmt_ms_to_tg(date_iso: Optional[str], time_hms: Optional[str]) -> str:
    ...
    try:
        ...
        return dt.tg.strftime("%d.%m.%Y %H:%M")
    except Exception:
        return raw
...
async def on_review_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if (
        ...
        await query.edit_message_text(
            "❌ Ma’lumot yetarli emas. Qaytadan /kiritish qiling.",
            reply_markup=None,
        )
        return ConversationHandler.END
    ...
    desc = (
        f"Counterparty: {cp.get('name')} | "
        f"Phone: {cp.get('phone') or ''} | "
    ...
    if GROUP_CHAT_ID and created:
        moment_ms = (created.get("moment") or "").strip()
        if moment_ms:
             try:
                  dt_ms = datetime.strptime(moment_ms[:19], "%Y-%m-%d %H:%M:%S").replace(tzinfo=MS_TZ)
                  moment_show = dt_ms.astimezone(TG_TZ).strftime("%d.%m.%Y %H:%M")
             except Exception:
                  moment_show = _fmt_ms_to_tg(date_iso, time_hms)
        else:
             moment_show = _fmt_ms_to_tg(date_iso, time_hms)

        doc_no = created.get("name") or created.get("code") or created.get("id") or "-"
        doc_title = "✅ Приходный ордер (черновик)" if pt == "cash" else "✅ Входящий платёж (черновик)"
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("вќЊ Bekor qilindi.", reply_markup=_menu_keyboard())
    _cleanup_after_done(context)
    return ConversationHandler.END
